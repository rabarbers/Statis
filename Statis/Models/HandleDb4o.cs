...
namespace Statis.Models
{
    public class HandleDb4o
    {
        ...
        /// Returns the Questionnaire objects present in DB40
        /// 
        ...
        ///
        public static IEnumerable<Question> GetQuestions(string questionnaireName)
        {
// TODO: TO BE IMPROVED FOR PERFORMANCE
        ...
        /// <returns></returns>
        public static Question GetQuestion(string questionText, string questionnaireName)
        {
            IObjectContainer db = GetDB();
            try
            {
                var question = (from Question q in questionnaireName
                                where q.Text == questionText
                                select q);
                // FirstOrDefault??
        ...
        public static IEnumerable<Choice> GetChoices(string option)
        {
            IObjectContainer db = GetDB();
            try
            {
                return new List<Choice>(from Choice ch in db
                                         where ch
                                         orderby ch.Name
                                         select ch).Distinct(new ProductByNameEquality());
